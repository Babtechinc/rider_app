...
import 'package:rider_app/Models/directionDetail.dart';
import 'package:rider_app/Models/placePrediction.dart';
...
//&components=country:ng
    var res = await RequestAssistant.getReguest(autoCompleteUrl);
    if (res == "failed") {
      return;
    }
    if(res["status"] =="OK" ){
...
class PredictionsTile extends StatelessWidget {
  ...
  const PredictionsTile({Key? key,this.placePredictions}) : super(key: key);
  ...
  void getPlaceAddressDetails(String? placeID,context) async{
    showDialog(context: context, builder: (BuildContext context)=> ProgressDialog(Message: "setting Dropff, please wait........")
    );
  String  placeDetailurl = 'https://maps.googleapis.com/maps/api/place/details/json?place_id=$placeID&key=$mapKey';
  var res = await RequestAssistant.getReguest(placeDetailurl);
   Navigator.pop(context);
  if (res == "failed") {
    return;
  }
  if(res["status"] =="OK" ){
    ...
    String  addressPlaceName = res["result"]["name"];
    double  latitude = res["result"]["geometry"]["location"]["lat"];
    double  longtitude = res["result"]["geometry"]["location"]["lng"];

    Address address = Address(placeName: addressPlaceName,placeID: placeID,latitude: latitude,longitude: longtitude);
